...
if [[ -n "${GH_PACKAGES_USER:-}" && -n "${GH_PACKAGES_TOKEN:-}" ]]; then
  ...
  cat <<'EOF'
Codespaces secrets GH_PACKAGES_USER and GH_PACKAGES_TOKEN were not found.
Create them in the repository or account Codespaces secrets and rebuild the codespace.
EOF
fi
